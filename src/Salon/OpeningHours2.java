...
public class OpeningHours2 {
    ...
    public boolean Ledig(LocalDateTime dt) {
        LocalDate date = dt.toLocalDate();
        LocalTime time = dt.toLocalTime();

        if (ferie.contains(date)) return false;
        if (date.getDayOfWeek() == DayOfWeek.SATURDAY) return false;
        if (date.getDayOfWeek() == DayOfWeek.SUNDAY) return false;
        return !time.isBefore(start)&&!time.isAfter(slut);//Tiden kan ikke være før åbning of efter luk, derfor kommer den frem til at det er under openinghours.
    }
}
